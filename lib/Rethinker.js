...
function saveRelationalData(savedData, relationalData) {
    ...
    function saveOrCreate(table, data) {
        ...
        return this.db.run(data.id ? table.get(data.id).update(data, {returnChanges: true}) : table.insert(data, {returnChanges: true}))
    }
            ...
            if (relData === null) {
                ...
                return saveOrCreate.call(self, relTable, relData)
                    .then(function (results) {
                        return saveRelationalData.call(self, results.changes[0].new_val, relationDef.with);
                    })
                    .then(function () {
                        return saveRelationAt.call(self, index + 1, modelData, relationalData);
                    });
                ...
                return saveOrCreate.call(self, relTable, relData)
                    .then(function (results) {
                        relData = results.changes[0].new_val;
                        var updateData = {};
...
function initModel() {
    ...
    this.prototype['create' + modelName] = function (insertData, opts) {
        var self = this,
            insertOpts = {validate: true, returnChanges : true},
            insertedVal,
            relationalData = [];
        ...
        return Promise.cast(insertOpts.validate ? self['validate' + modelName](insertData, 'create') : true)
            ...
            .then(function (proceedToSave) {
                ...
                return self.db.run(table.insert(insertData, {returnChanges: Array.isArray(insertData) ? false : insertOpts.returnChanges}));
            ...
            .then(function (result) {
                ...
                if(result.inserted === 1){
                    insertOpts.returnChanges && (insertedVal = result.changes[0].new_val);
    ...
    this.prototype['update' + modelName] = function (updateData, queryData, opts) {
        var self = this,
            ...
            updatedVal,
            relationalData = [],
            updateOpts = {validate: true, returnChanges : true};
        ...
        return Promise.cast(updateOpts.validate ? self['validate' + modelName](updateData, 'update') : true)
            ...
            .then(function (proceedToSave) {
                ...
                if (updateOpts.returnChanges) { //temporary patch that returnChanges doesnt support multiple-row selection
                    updateQuery = getPrimaryKey(queryData, opts) ?
                        self.buildQuery(queryData, opts).update(updateData, {returnChanges: true}) :
                        self.buildQuery(queryData, _.assign({}, {orderBy: 'id', limit: 1}, updateOpts)).forEach(function (doc) {
                            return table.get(doc('id')).update(updateData, {returnChanges: true});
                        });
                } else {
            ...
            .then(function (result) {
                if (!result) return null;
                if (updateOpts.returnChanges) {
                    updatedVal = result.changes[0].new_val;
                ...
                return Promise.cast(saveRelationalData.call(self, updatedVal, relationalData))
                    ...
                    .then(function (savedVal) {
                        var oldChanges = result.changes;
                        if (oldChanges && oldChanges.length === 1){
                            oldChanges = oldChanges[0].old_val;
                        }
                        savedVal && (savedVal = self['afterUpdate' + modelName](savedVal, oldChanges) || savedVal);
                        return savedVal;
                    });
    ...
    this.prototype['updateAll' + modelName] = function(updateData, queryData, opts){
        var self = this;
        return self['findAll' + modelName](queryData, opts)
            .then(function(models){
                var updatePromises = [];
                for (var i = 0, ll = models.length; i < ll; i++) {
                  var model = models[i];
                  updatePromises.push(self['update' + modelName](updateData, model.id, opts));
                }
